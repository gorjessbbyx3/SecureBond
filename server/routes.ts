...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Staff endpoint for inquiry assignment (admin-only)
  ...
  // Contact inquiry routes (from Supabase contact form) - ADMIN ONLY
  app.get('/api/inquiries', isAuthenticated, async (req, res) => {
    try {
      ...
      if (!supabase) {
        return res.status(200).json({ disabled: true, message: "Contact inquiries unavailable - Supabase not configured", data: [] });
      }
      ...
      res.json({ disabled: false, data: data || [] });
    } catch (error) {
  ...
  app.delete('/api/inquiries/:id', isAuthenticated, async (req, res) => {
    try {
      ...
      if (!supabase) {
        return res.status(503).json({ disabled: true, message: "Supabase not configured" });
      }
